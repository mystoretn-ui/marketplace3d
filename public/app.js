...
async function showModel(id) {
  ...
  $("#modalContent").innerHTML = `
    ...
    <p class="muted">
      Designer : ${escapeHtml(m.designerName)}
    </p>

    <p>
      ${escapeHtml(m.description || "Aucune description.")}
    </p>
...
async function buy(id) {
  ...
  try {
    const r = await api("/api/purchase/" + id, {
      method: "POST"
    });

    alert(
      "Démo : achat enregistré. Le paiement réel doit être connecté avant la mise en production."
    );
