...
function openUpload(){
  ...
  <form onsubmit="uploadModel(event)">
    ...
    <div class="priceOptions">
      <label><input type="radio" name="pricing" value="paid" checked onchange="togglePriceMode()"> Payant</label>
      <label><input type="radio" name="pricing" value="free" onchange="togglePriceMode()"> Gratuit</label>
    </div>
    <input id="priceInput" name="price" type="number" min="0.01" step="0.01" placeholder="Prix en$" value="1">
...
function togglePriceMode(){
  const free = document.querySelector('input[name="pricing"]:checked')?.value === "free";
  const input = $("#priceInput");
  if (!input) return;
  input.disabled = free;
  input.required = !free;
  input.value = free ? "0" : (Number(input.value) > 0 ? input.value : "1");
  input.placeholder = free ? "Gratuit" : "Prix en$";
}
